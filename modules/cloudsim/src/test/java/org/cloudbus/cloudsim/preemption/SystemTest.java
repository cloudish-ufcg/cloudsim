...
//        // verify expected availability for the vms
    ...
    @Test
    public void testSystemSingleHostWithAvalabilityAwarePolicyNewTrace() {
        ...
        advanceTime(0.0);
        //allocating vms with submit time 0
        testSimulationRuntime0NewTrace();

        advanceTime(1.0);
        //allocating vms with submit time 1
        testSimulationRuntime1NewTrace();

        advanceTime(5.0);
        testSimulationRuntime5NewTrace();

        advanceTime(6.0);
        testSimulationRuntime6NewTrace();

        advanceTime(7.0);
        testSimulationRuntime7NewTrace();

        advanceTime(8.0);
        testSimulationRuntime8NewTrace();
    ...
    private void submitEventsNewTrace() {
        ...
        for (int i = 0; i < NUMBER_OF_VMS_P0; i++) {
            ...
            CloudSim.send(datacenter.getId(), datacenter.getId(), vm.getSubmitTime(), CloudSimTags.VM_CREATE, vmP0S0.get(i));
        }
        ...
        for (int i = 0; i < NUMBER_OF_VMS_P1; i++) {
            ...
            CloudSim.send(datacenter.getId(), datacenter.getId(), vm.getSubmitTime(), CloudSimTags.VM_CREATE, vmP1S0.get(i));
        }
        ...
        for (int i = 0; i < NUMBER_OF_VMS_P2; i++) {
            ...
            CloudSim.send(datacenter.getId(), datacenter.getId(), vm.getSubmitTime(), CloudSimTags.VM_CREATE, vmP2S0.get(i));
        }


    }

    private void testSimulationRuntime0NewTrace() {
    ...
    private void testSimulationRuntime1NewTrace() {
    ...
    private void testSimulationRuntime5NewTrace() {

        for(int i = 10; i < vmP1S0.size(); i++) {
    ...
    private void testSimulationRuntime6NewTrace() {
    ...
    private void testSimulationRuntime7NewTrace() {
        ...
        PreemptableVm vm = (PreemptableVm) vmP0S0.get(9);
        Assert.assertEquals(1.0, vm.getCurrentAvailability(7.0), ACCEPTABLE_DIFFERENCE);
        Assert.assertEquals(0, vm.getNumberOfPreemptions());
    ...
    private void testSimulationRuntime8NewTrace() {
